...
#ifdef OPLUS_FEATURE_CHG_BASIC
	pr_debug(KERN_ERR "!!!!! smblib_usb_plugin_hard_reset_locked: [%d]\n", vbus_rising);
#endif
...
static void smblib_uusb_otg_work(struct work_struct *work)
{
	...
	if (otg_status ^ otg) {
		otg_status = otg;
		pr_debug(KERN_ERR "!!!!! smblib_handle_usb_typec_change_for_uusb: otg status [%d]\n", otg_status);
	}
#endif
...
static void oplus_ccdetect_disable(void)
{
	...
	{
		pr_debug(KERN_ERR "[OPLUS_CHG][%s]: ccdetect is in FTM mode, enable otg!\n", __func__);
		value = true;
	}
	if (g_oplus_chip && g_oplus_chip->vbatt_num == 2) {
		otg_disable_id_value();
...
int pm660l_bob_regulator_get_mode(unsigned int *mode)
{
	...
	if (!g_oplus_chip) {
		pr_debug(KERN_ERR "pm660l_bob_regulator_get_mode: g_oplus_chip NULL\n");
		return -1;
	}
	chg = &g_oplus_chip->pmic_spmi.smb2_chip->chg;
