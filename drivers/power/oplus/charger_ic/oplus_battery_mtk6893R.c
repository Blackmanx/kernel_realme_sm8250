...
static int oplus_mt6360_charging_current_write_fast(int chg_curr)
{
	...
	if (!g_oplus_chip->authenticate) {
		...
		pr_debug(KERN_ERR "[OPLUS_CHG][%s]:!g_oplus_chip->authenticate , charging_disable\n", __func__);
		return 0;
	}
...
static void oplus_mt6360_set_aicl_point(int vbatt)
{
	...
	if (!g_oplus_chip->authenticate) {
		pr_debug(KERN_ERR "[OPLUS_CHG][%s]:!g_oplus_chip->authenticate \n", __func__);
		return;
	}
...
bool oplus_mt_get_vbus_status(void)
{
	...
	if (ret < 0) {
		if (g_oplus_chip && g_oplus_chip->unwakelock_chg == 1
			...
			pr_debug(KERN_ERR "[OPLUS_CHG][%s]: unwakelock_chg=1, use pre status\n", __func__);
...
int oplus_mt6360_pd_setup_forvoocphy(void)
{
	...
	pr_debug(KERN_ERR "%s: pd_type %d pd9v svooc [%d %d %d %d %d]", __func__, pinfo->pd_type, chip->limits.vbatt_pdqc_to_9v_thr, chip->limits.vbatt_pdqc_to_5v_thr, chip->batt_volt, chip->camera_on,chip->charger_volt);
	...
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90
		...
		if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
			...
			for (i = 0; i < cap.nr; i++) {
				pr_debug(KERN_ERR "PD APDO cap %d: mV:%d,%d mA:%d type:%d pwr_limit:%d pdp:%d\n", i,
			...
			for (i = 0; i < cap.nr; i++) {
				pr_debug(KERN_ERR "PD cap %d: mV:%d,%d mA:%d type:%d\n", i,
		...
		pr_debug(KERN_ERR "PD request: %dmV, %dmA\n", vbus_mv, ibus_ma);
		...
		pr_debug(KERN_ERR "%s: pd9v svooc  default[%d %d]", __func__, em_mode, chip->batt_volt);
	}
...
int oplus_mt6360_pd_setup_forsvooc(void)
{
	...
	if (!chip->calling_on && chip->charger_volt < 6500 && chip->soc < 90
		...
		if (is_vooc_support_single_batt_svooc() == true) {
			...
			ret = oplus_pdc_setup(&vbus_mv, &ibus_ma);
			pr_debug(KERN_ERR "%s: PD request: vbus[%d], ibus[%d], ret[%d]\n", __func__, vbus_mv, ibus_ma, ret);
			msleep(300);
...
void oplus_chg_choose_gauge_curve(int index_curve)
{
	...
	if (target_index_curve != last_curve_index) {
		pr_debug(KERN_ERR "%s: index_curve() =%d	target_index_curve =%d last_curve_index =%d  pd svooc %s",
					__func__, index_curve, target_index_curve, last_curve_index,
					chip->pd_svooc == true ?"true":"false");

		pr_debug(KERN_ERR "%s svooc_30w_adapter %s set_charge_power_sel  %d", __func__,
...
int oplus_chg_set_qc_config_forvoocphy(void)
{
	...
	chip->charger_volt = battery_meter_get_charger_voltage();
	pr_debug(KERN_ERR "%s: qc9v svooc [%d %d %d %d %d]", __func__, chip->limits.vbatt_pdqc_to_9v_thr, chip->limits.vbatt_pdqc_to_5v_thr, chip->batt_volt, chip->camera_on, chip->charger_volt);
	chip->charger_volt = chip->chg_ops->get_charger_volt();
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90 && chip->temperature <= 420 && chip->cool_down_force_5v == false) {
		...
		pr_debug(KERN_ERR "%s: qc9v svooc  default[%d %d]", __func__, em_mode, chip->batt_volt);
	}
