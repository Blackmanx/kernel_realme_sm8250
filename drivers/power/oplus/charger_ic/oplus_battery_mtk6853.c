	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_WATER_DETECT_FEATURE:
			pr_debug(KERN_ERR "[OPLUS_CHG][%s]: oplus_set_water_detect[%d]\n", __func__, val->intval);
			if (val->intval == 0) {
		...
		if (is_vooc_project()) {
			switch (psp) {
				...
				case POWER_SUPPLY_PROP_FIXED_FREQ:
					...
					pr_debug(KERN_ERR "[OPLUS_CHG][%s]: intval [%d] charger_exist[%d] charge_pump_mode[%d]\n", __func__, val->intval, g_oplus_chip->charger_exist, charge_pump_mode);
					break;
				default:
					rc = oplus_battery_set_property(psy, psp, val);
					break;
...
int oplus_mt6360_pd_setup(void)
{
	...
	if(!chip) {
		pr_debug(KERN_ERR "%s:g_oplus_chip not ready\n", __func__);
		return -1;
	}
